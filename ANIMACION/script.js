...
const estilo = [65, 33, 93, 79, -44,290] //93,65,33,79
let estiloAleatorio = 25
...
let i = 0;
function girar() {
    let grados = estiloAleatorio * (Math.PI / 180)
    canvas.rotate(grados)
    if (i == 175) { //175
        const n = () => Math.floor(Math.random() * estilo.length)
        estiloAleatorio = estilo[n()]
        let rgb1 = aleatorio()
        let rgb2 = aleatorio()
        let rgb3 = aleatorio()
    ...
    canvas.strokeRect(-posX / 2, -posY / 2, 270, 270) //270
